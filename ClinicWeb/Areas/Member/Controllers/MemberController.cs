...
namespace ClinicWeb.Areas.Member.Controllers
{
	...
	public class MemberController : Controller
	{
		...
		/*, [FromBody] string GenderString, [FromBody] string VerificationString*/
		[HttpPost]
		[ValidateAntiForgeryToken]
		public IActionResult MemberCreate([FromBody] MemberViewModel member)
		{
			//測試用 驗證資料是否進入 是否為空值
			if (member == null)
			{
			...
			if (!ModelState.IsValid || member == null)
			{
				...
				return PartialView("~/Areas/Member/Views/Partial/_MemberCreatePartial.cshtml", member);
			}
			//加入資料庫
			int maxMemberNumber = _context.MemberMemberList.Max(m => m.MemberNumber);
			var nextMemberNumber = maxMemberNumber + 1;

			_context.MemberMemberList.Add(new MemberMemberList
			{
				...
				MemPassword = member.MemPassword,
				Verification = member.Verification,
				BirthDate = (DateTime)member.BirthDate
			});
			_context.SaveChanges();

			return Content("Success");
		}
		public IActionResult MemberCreatePartial()
		{
